...
To implement durable execution in LangGraph, you need to:

1. Enable [**persistence**](./persistence.md) in your workflow by specifying a [**checkpointer**](./persistence.md#checkpoints) that will save workflow progress.
...
For some examples of pitfalls to avoid, see the [Functional API Common Pitfalls](./functional_api.md#common-pitfalls) section, which shows
how to structure your code using **tasks** to avoid these issues. **tasks** can be used from within nodes (Graph API) or entrypoints (Functional API).
...
## Starting Points for Resuming Workflows (Advanced)
