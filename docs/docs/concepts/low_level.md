...
Consider using [conditional edges](#conditional-edges) to route between nodes conditionally without updating the state. Although this can be achieved with `Command(goto="node")`, overusing goto statements can make it difficult to debug and make the application’s control flow harder to understand.
