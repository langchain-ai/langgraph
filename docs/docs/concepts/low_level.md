...
!!! important "State updates with `Command.PARENT`"
    ...
    When you send state updates from a subgraph node to a parent graph node, you **must** define a [reducer](#reducers) for the key you're updating in the parent graph state. See this [example](../how-tos/command.ipynb#navigating-to-a-node-in-a-parent-graph).
