...
To add a sequence of nodes, we use the `.add_node` and `.add_edge` methods of our [graph](../../concepts/low_level/#stategraph.md):
```python
...
Let's first define our [state](../../concepts/low_level/#state.md). This governs the [schema of the graph](../../concepts/low_level/#schema.md), and can also specify how to apply updates. See [this guide](../../how-tos/state-reducers.md) for more detail.
...
Our [nodes](../../concepts/low_level/#nodes.md) are just Python functions that read our graph's state and make updates to it. The first argument to this function will always be the state:
...
By default, this will **overwrite** the value of the corresponding key. You can also use [reducers](../../concepts/low_level/#reducers.md) to control how updates are processed— for example, you can append successive updates to a key instead. See [this guide](../../how-tos/state-reducers.md) for more detail.

### Define graph

We use [StateGraph](../../concepts/low_level/#stategraph.md) to define a graph that operates on this state.

We will then use [add_node](../../concepts/low_level/#messagesstate.md) and [add_edge](../../concepts/low_level/#edges.md) to populate our graph and define its control flow.
...
- We must specify the entry point of the graph. For this we add an edge with the [START node](../../concepts/low_level/#start-node.md).
- The graph halts when there are no more nodes to execute.

We next [compile](../../concepts/low_level/#compiling-your-graph.md) our graph. This provides a few basic checks on the structure of the graph (e.g., identifying orphaned nodes). If we were adding persistence to our application via a [checkpointer](../../concepts/persistence/.md), it would also be passed in here.
