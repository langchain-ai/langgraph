...
#if TYPE_CHECKING: -> Generates import error
...
class DynamoDbSaver(BaseCheckpointSaver):
    ...
    def setup(self) -> None:
        ...
        try:
            ...
            logger.info(f"Table {table_name} created successfully with a Global Secondary Index on timestamp.")
    ...
    def get(self, config: RunnableConfig) -> Optional[Checkpoint]:
        logger.debug(f"get - Retrieving checkpoint for thread ID: {config['configurable']['thread_id']}")
        self.setup()
        ...
        if item and self.checkpoint_field_name in item:
            ...
            logger.debug(f"get - Retrieved checkpoint for thread ID: {config['configurable']['thread_id']}")
            logger.debug(f"get - Checkpoint: {checkpoint}")
            return checkpoint
        else:
            return None

    def put(self, config: RunnableConfig, checkpoint: Checkpoint) -> None:
        logger.debug(f"put - Saving checkpoint for thread ID: {config['configurable']['thread_id']}")
        self.setup()
        self.table.put_item(
            Item={
                self.primary_key_name: config["configurable"]["thread_id"],
