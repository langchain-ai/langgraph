...
async def run_agent_with_checkpoints():
    ...
    if not cluster_uri:
        ...
        logger.info(
            "Set it with: export KUSTO_CLUSTER_URI=https://your-cluster.region.kusto.windows.net"
        )
        return False
        ...
        async with AsyncKustoSaver.from_connection_string(
            ...
            try:
                ...
                if loaded:
                    ...
                    logger.info(
                        f"  Message count: {len(loaded.checkpoint.get('channel_values', {}).get('messages', []))}"
                    )
                else:
                ...
                if final_checkpoint:
                    messages = final_checkpoint.checkpoint.get(
                        "channel_values", {}
                    ).get("messages", [])
