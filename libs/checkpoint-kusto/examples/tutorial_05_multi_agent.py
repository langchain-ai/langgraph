...
class ResearchState(TypedDict):
    ...
    research_phase: str
    findings: Annotated[
        dict[str, str], merge_findings
    ]  # Now can handle parallel updates
    research_history: Annotated[
        dict[str, list[str]], merge_research_history
    ]  # Persistent across sessions


def create_specialist_agent(
    name: str, specialty: str, llm: ChatOpenAI | AzureChatOpenAI
):
    ...
    async def agent_function(state: ResearchState) -> ResearchState:
        ...
        if other_findings:
            context_parts.append(
                f"Current session - Other team members found:\n{other_findings}"
            )
        ...
        if agent_history:
            context_parts.append(
                ...
                + "\n".join(
                    [f"  - {finding[:100]}..." for finding in agent_history[-3:]]
                )  # Last 3
            )
...
async def synthesizer_agent(
    state: ResearchState, llm: ChatOpenAI | AzureChatOpenAI
) -> ResearchState:
...
def create_research_graph(llm: ChatOpenAI | AzureChatOpenAI) -> StateGraph:
    ...
    business_agent = create_specialist_agent(
        "business_analyst", "business value, market fit, and ROI", llm
    )

    ux_agent = create_specialist_agent(
        "ux_researcher", "user experience, usability, and adoption", llm
    )
            ...
            if checkpoint:
                ...
                if previous_history:
                    ...
                    for agent, findings in previous_history.items():
                        print(
                            f"   {agent.replace('_', ' ').title()}: {len(findings)} previous finding(s)"
                        )
                    print()
            ...
            if checkpoint:
                ...
                if final_history:
                    ...
                    for agent, findings_list in final_history.items():
                        print(
                            f"   {agent.replace('_', ' ').title()}: {len(findings_list)} total finding(s)"
                        )
