...
class TestMongoDBSaver:
    """Test class for testing MongoDBSaver functionality including the `aput` and `alist` methods for storing and searching checkpoints."""
    ...
    @pytest.fixture
    async def mongo_saver(self):
        """
        Fixture for creating an instance of MongoDBSaver.
        """
        async with AsyncMongoDBSaver.from_conn_info(
            self.conn_string, self.db_name
        ) as saver:
            yield saver

    async def test_asearch(self, mongo_saver: AsyncMongoDBSaver) -> None:
        """
        Test the `alist` and `aput` methods of MongoDBSaver for storing and searching checkpoints.
        Verifies the correctness of search queries, including different filters and configurations.
        """
        await mongo_saver.aput(self.config_1, self.chkpnt_1, self.metadata_1, {})
        await mongo_saver.aput(self.config_2, self.chkpnt_2, self.metadata_2, {})
        await mongo_saver.aput(self.config_3, self.chkpnt_3, self.metadata_3, {})
