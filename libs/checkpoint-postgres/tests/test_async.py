...
class TestAsyncPostgresSaver:
    ...
    async def test_asearch(self):
        # set up test
        # save checkpoints
        await self.saver.aput(self.config_1, self.chkpnt_1, self.metadata_1)
        await self.saver.aput(self.config_2, self.chkpnt_2, self.metadata_2)
        await self.saver.aput(self.config_3, self.chkpnt_3, self.metadata_3)
