    ...
    async def aget_tuple(self, config: RunnableConfig) -> CheckpointTuple | None:
        ...
        async with self.lock, self.conn.cursor() as cur:
            ...
            if checkpoint_id := get_checkpoint_id(config):
                ...
                await cur.execute(
                    "SELECT thread_id, checkpoint_id, parent_checkpoint_id, type, checkpoint, metadata FROM checkpoints WHERE thread_id = ? AND checkpoint_ns = ? ORDER BY rowid DESC LIMIT 1",
