"""Smoke tests for `BaseCheckpointSaver.get_delta_history` on sqlite.
...
* When a `_DeltaSnapshot` blob is present at an ancestor, it is returned
  as the `seed`.
* The async saver returns the same shape via `aget_delta_history`.
"""
...
def _pick_non_root(saver: Any, config: RunnableConfig) -> RunnableConfig:
    ...
    `get_delta_history` walks the parent chain — calling it on the root
...
def test_empty_channels_returns_empty_mapping_sync() -> None:
    ...
    with SqliteSaver.from_conn_string(":memory:") as saver:
        ...
        assert saver.get_delta_history(config=config, channels=[]) == {}
...
def test_writes_history_oldest_to_newest_sync() -> None:
    ...
    with SqliteSaver.from_conn_string(":memory:") as saver:
        ...
        result = saver.get_delta_history(config=target_cfg, channels=["items"])
...
def test_seed_present_when_snapshot_in_ancestor_sync() -> None:
    ...
    with SqliteSaver.from_conn_string(":memory:") as saver:
        ...
        # `_DeltaSnapshot`. After this surgery, calling `get_delta_history`
        ...
        result = saver.get_delta_history(config=leaf_tup.config, channels=["items"])
        entry = result["items"]
...
def test_seed_omitted_when_walk_reaches_root_sync() -> None:
    """`get_delta_history` on the root checkpoint → no `seed` key, no writes."""
    with SqliteSaver.from_conn_string(":memory:") as saver:
        ...
        result = saver.get_delta_history(config=root_tup.config, channels=["items"])
        entry = result["items"]
...
async def test_empty_channels_returns_empty_mapping_async() -> None:
    ...
    async with AsyncSqliteSaver.from_conn_string(":memory:") as saver:
        ...
        assert await saver.aget_delta_history(config=config, channels=[]) == {}
...
async def test_writes_history_oldest_to_newest_async() -> None:
    ...
    async with AsyncSqliteSaver.from_conn_string(":memory:") as saver:
        ...
        result = await saver.aget_delta_history(config=target_cfg, channels=["items"])
...
async def test_seed_omitted_when_walk_reaches_root_async() -> None:
    ...
    async with AsyncSqliteSaver.from_conn_string(":memory:") as saver:
        ...
        result = await saver.aget_delta_history(
