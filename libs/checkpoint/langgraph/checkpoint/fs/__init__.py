...
class FileSystemSaver(
    ...
    """A filesystem checkpoint saver.

    This checkpoint saver persists data to disk within a given folder using PersistentDict.

    Note:
        Only use `FileSystemSaver` for debugging or testing purposes.
    ...
    Args:
        ...
        base_path: The base directory where checkpoint data will be stored. This directory will be created if it doesn't exist.

    Examples:

            import asyncio

            from langgraph.checkpoint.memory import FileSystemSaver
            ...
            memory = FileSystemSaver(base_path="./checkpoints")
    ...
    def __init__(self, base_path: str, *, serde: SerializerProtocol | None = None) -> None:
        super().__init__(serde=serde)
        os.makedirs(base_path, exist_ok=True)
        self.storage = PersistentDict(lambda: defaultdict(dict), filename=os.path.join(base_path, 'storage.db'))
        self.storage.load()
        self.writes = PersistentDict(lambda: dict(), filename=os.path.join(base_path, 'writes.db'))
        self.writes.load()
        self.blobs = PersistentDict(filename=os.path.join(base_path, 'blobs.db'))
        self.blobs.load()
        self.stack = ExitStack()

    def __enter__(self) -> FileSystemSaver:
        return self
    ...
    async def __aenter__(self) -> FileSystemSaver:
        return self
    ...
    def get_tuple(self, config: RunnableConfig) -> CheckpointTuple | None:
        """Get a checkpoint tuple from the filesystem storage.
    ...
    def list(
        ...
        *,
        ...
        """List checkpoints from the filesystem storage.
    ...
    def put(
        ...
        """Save a checkpoint to the filesystem storage.
    ...
    def put_writes(
        ...
        """Save a list of writes to the filesystem storage.
...
MemorySaver = None  # deprecated
FilesystemSaver = FileSystemSaver  # alias for filesystem-based saver
