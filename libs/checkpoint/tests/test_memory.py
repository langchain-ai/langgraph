...
class TestInMemorySaverDeltaChannel:
    ...
    def test_get_channel_writes_collects_ancestor_writes_only(self) -> None:
        """`get_delta_writes` collects ancestor writes oldest→newest, and
        ...
        result = saver.get_delta_writes(config=config, channels=[channel])[channel]
...
class TestBaseFallbackGetChannelWrites:
    """Exercises the `BaseCheckpointSaver.get_delta_writes` default
    implementation — the path third-party savers inherit when they don't
    override `get_delta_writes` themselves.
    ...
    def _build_saver_with_chain(self) -> tuple[InMemorySaver, str, str]:
        ...
        class _ThirdPartyStyleSaver(InMemorySaver):
            get_delta_writes = (
                InMemorySaver.__mro__[1].get_delta_writes  # type: ignore[attr-defined]
            )
            aget_delta_writes = (
                InMemorySaver.__mro__[1].aget_delta_writes  # type: ignore[attr-defined]
            )
    ...
    def test_fallback_returns_ancestor_writes_oldest_first(self) -> None:
        ...
        result = saver.get_delta_writes(config=config, channels=["messages"])[
            "messages"
        ]
    ...
    async def test_async_fallback_returns_ancestor_writes_oldest_first(self) -> None:
        ...
        result = (await saver.aget_delta_writes(config=config, channels=["messages"]))[
            "messages"
        ]
    ...
    async def test_async_fallback_concurrent_tasks_do_not_interfere(self) -> None:
        ...
        Two concurrent `aget_delta_writes` calls on the same event-loop
        ...
        results = await asyncio.gather(
            saver.aget_delta_writes(config=config, channels=["messages"]),
            saver.aget_delta_writes(config=config, channels=["messages"]),
        )
...
class TestPreDeltaBlobTerminator:
    ...
    `DeltaWrites` TypedDict; absence of the `seed` key means the walk
