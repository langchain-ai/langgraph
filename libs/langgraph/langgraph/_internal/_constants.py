...
CONFIG_KEY_SUBGRAPH_KEY = sys.intern("__pregel_subgraph_key")
# internal transport for `Send(key=...)`: set on the pushed task's config and
# read by the subgraph it invokes. Holds a key identifying a subgraph *instance*. When set, the
