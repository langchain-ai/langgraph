...
class Analyzer:
    ...
    digraph = nx.DiGraph()
    condition_nodes_name = []
    condition_nodes_children = []
    ...
    def find_connected_nodes(self):
        """
        find all condition nodes' children
        """

        # store children
        condition_nodes_children = set()

        # get all condition nodes' children
        for node_id in self.condition_nodes_name:
            all_descendants = set(nx.descendants(self.digraph, node_id))
            condition_nodes_children.update(all_descendants)

        self.condition_nodes_children = list(condition_nodes_children)

        return self.condition_nodes_children
