    ...
    **Actors** read data from channels and write data to channels.
    ...
    def bulk_update_state(
        ...
        def perform_superstep(
            ...
            node_to_task_ids: dict[str, list[str]] = defaultdict(list)
            # keep track of how many task ids have been reused for each node.
            # task ids can be provided in the StateUpdate, but if not,
            # we use the task id generated by prepare_next_tasks
            node_to_num_task_ids_reused: dict[str, int] = defaultdict(int)
            ...
            for as_node, values, provided_task_id in valid_updates:
                ...
                prepared_task_ids = node_to_task_ids.get(as_node, [])
                task_id = provided_task_id or str(
                    prepared_task_ids[node_to_num_task_ids_reused[as_node]]
                    if node_to_num_task_ids_reused[as_node] < len(prepared_task_ids)
                    else uuid5(UUID(checkpoint["id"]), INTERRUPT)
                )
                node_to_num_task_ids_reused[as_node] += 1
