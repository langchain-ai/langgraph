...
Mechanism under test: the saver's public `get_delta_writes(config,
...
`seed` in the returned `DeltaWrites`. If the walk reaches the root
...
4. **Base-saver fallback path**: a third-party-style subclass that
   ...
   `BaseCheckpointSaver.get_delta_writes` must produce the same
...
class _ThirdPartyStyleSaver(InMemorySaver):
    ...
    `get_delta_writes` implementation from `BaseCheckpointSaver`
    ...
    get_delta_writes = (  # type: ignore[assignment]
        InMemorySaver.__mro__[1].get_delta_writes  # type: ignore[attr-defined]
    )
    aget_delta_writes = (  # type: ignore[assignment]
...
def test_delta_and_migrated_threads_do_not_cross_contaminate() -> None:
    ...
    `get_delta_writes` must be scoped to the target thread.
    """
