...
def test_react_agent_with_structured_response() -> None:
    """Test that create_react_agent can extract structured output from AI responses."""
    ...
    assert structured_response.temperature == 70.0
    assert structured_response.wind_direction == "NE"
    assert structured_response.wind_speed == 5.0
    
    # Verify we have the expected messages (human, ai with tool call, tool message, ai with final response)
    messages = result["messages"]
    assert len(messages) >= 4
    assert isinstance(messages[0], HumanMessage)
    assert isinstance(messages[-1], AIMessage)
    assert messages[-1].content == '{"temperature": 70.0, "wind_direction": "NE", "wind_speed": 5.0}'
