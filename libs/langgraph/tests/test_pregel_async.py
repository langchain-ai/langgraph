...
async def test_stream_mode_messages_with_messages_key_filter() -> None:
    ...
    async def public_node(state: State) -> State:
        return {"messages": [AIMessage(content="Public response")]}

    async def internal_node(state: State) -> State:
    ...
    all_contents = [
        message.content
        async for message, _ in app.astream(state, stream_mode="messages")
    ]
    filtered_runtime_contents = [
        message.content
        async for message, _ in app.astream(
            state, stream_mode="messages", messages_key="messages"
        )
    ]
    filtered_compile_contents = [
        message.content
        async for message, _ in app_with_filter.astream(state, stream_mode="messages")
    ]
