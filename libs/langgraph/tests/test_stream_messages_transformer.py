...
class TestAsyncMode:
    ...
    async def test_output_awaitable(self) -> None:
        ...
        for evt in _lifecycle(text="async"):
            t.process(_proto_event(evt))
        (stream,) = list(log._items)
        assert (await stream.output).content == "async"
...
class TestViaMux:
    ...
    def test_streaming_via_mux(self) -> None:
        ...
        mux.close()
        assert list(log._items)[0].output.content == "mux stream"

    def test_whole_message_via_mux(self) -> None:
        t, mux, log = self._make_mux()
        mux.push(_whole_msg("result"))
        mux.close()
        assert list(log._items)[0].output.content == "result"
    ...
    async def test_async_streaming_via_mux(self) -> None:
        ...
        (stream,) = list(log._items)
        assert (await stream.output).content == "async mux"
        await mux.aclose()
...
class TestEndToEnd:
    ...
    def test_text_deltas_iterate(self) -> None:
    ...
    def test_non_llm_message_returned_from_node(self) -> None:
        ...
        run = graph.stream_v2({"messages": "hi"})
        (stream,) = list(run.messages)
        assert stream.output.content == "hardcoded"
    ...
    async def test_async_node_calling_astream_v2(self) -> None:
        ...
        assert (await streams[0].output).content == "async answer"
...
class TestEndToEndV2Invoke:
    ...
    def test_invoke_emits_protocol_events(self) -> None:
        ...
        (stream,) = list(run.messages)
        event_types = {e.get("event") for e in stream}
        assert event_types >= {
            "message-start",
            "content-block-start",
            "content-block-delta",
    ...
    def test_invoke_two_nodes_two_streams(self) -> None:
        ...
        assert len(streams) == 2
        assert {s.output.content for s in streams} == {"alpha", "beta"}
    ...
    def test_invoke_plus_constructed_message_two_streams(self) -> None:
        ...
        assert len(streams) == 2
        assert (
            streams[0].node == "streaming_node"
            and streams[0].output.content == "live stream"
        )
        assert (
            streams[1].node == "constructed_node"
            and streams[1].output.content == "hardcoded"
        )
    ...
    async def test_ainvoke_populates_messages(self) -> None:
        ...
        assert len(streams) == 1
        assert isinstance(streams[0], AsyncChatModelStream)
        assert (await streams[0].output).content == "async invoke"
