...
async def test_tool_node_timeout_returns_error_message() -> None:
    ...
    tool_node = ToolNode([slow_tool], timeout=0.1, handle_tool_errors=True)
...
async def test_tool_node_timeout_parallel_tools() -> None:
    ...
    tool_node = ToolNode([slow_tool, fast_tool], timeout=0.5, handle_tool_errors=True)
    msg = AIMessage(
        "",
        tool_calls=[
    ...
    assert statuses["fast_tool"] != "error"
