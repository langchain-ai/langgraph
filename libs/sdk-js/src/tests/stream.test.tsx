...
describe("useStream onStop callback", () => {
  ...
  it("mutate function updates stream values immediately", async () => {
    ...
    // Initial state
    expect(screen.getByTestId("stopped-status")).toHaveTextContent("Not stopped");
    expect(screen.getByTestId("custom-message")).toHaveTextContent("No message");
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("custom-message")).toHaveTextContent("Stream stopped");
    });
  });
  ...
  it("handles functional updates correctly", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("items")).toHaveTextContent("item1, item2, stopped");
    });
  });
