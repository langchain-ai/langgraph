"""Tests for skip_auto_load_api_key parameter."""
...
class TestSkipAutoLoadApiKey:
    """Test the skip_auto_load_api_key parameter."""
    ...
    @pytest.mark.asyncio
    async def test_get_client_skips_env_when_flag_set(self, monkeypatch):
        """Test that API key is not loaded from environment when skip flag is True."""
        monkeypatch.setenv("LANGGRAPH_API_KEY", "test-key-from-env")

        client = get_client(url="http://localhost:8123", skip_auto_load_api_key=True)
    ...
    async def test_get_client_uses_explicit_key_when_provided(self, monkeypatch):
        ...
        # Even with skip_auto_load_api_key=True, explicit key should be used
        client = get_client(
    ...
    async def test_get_client_no_key_when_skip_and_no_env(self, monkeypatch):
        """Test that no API key is set when skip is True and no explicit key."""
    ...
    def test_get_sync_client_skips_env_when_flag_set(self, monkeypatch):
        """Test that sync client doesn't load from environment when skip flag is True."""
        monkeypatch.setenv("LANGGRAPH_API_KEY", "test-key-from-env")

        client = get_sync_client(
            url="http://localhost:8123", skip_auto_load_api_key=True
        )
        assert "x-api-key" not in client.http.client.headers
        client.close()
