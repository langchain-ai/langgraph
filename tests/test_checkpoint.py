...
def test_dynamodb_init(aws_credentials):
    ...
    assert str(dynamodb.client.__class__) == str(boto3_session.resource("dynamodb", endpoint_url=endpoint_url).__class__)
    assert str(dynamodb.table.__class__) == str(dynamodb.client.Table(table_name).__class__)
    ...
    assert dynamodb.primary_key_name == primary_key_name
    assert dynamodb.checkpoint_field_name == checkpoint_field_name
    assert str(dynamodb.client.__class__) == str(boto3.resource("dynamodb", endpoint_url=endpoint_url).__class__)
    assert str(dynamodb.table.__class__) == str(dynamodb.client.Table(table_name).__class__)
